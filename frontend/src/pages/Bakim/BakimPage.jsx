...
export default function BakimPage() {
  ...
  // Sesle rapor (STT) + AI ile düzenleme (NLP) durumu
  const [dinliyor, setDinliyor] = useState(false);
  const [raporDuzenleniyor, setRaporDuzenleniyor] = useState(false);
  const recognitionRef = useRef(null);
  // Tarayıcı konuşma tanıma desteği var mı? (Chrome/Edge'de var)
  const sesDestekli = typeof window !== 'undefined' &&
  ...
  // Mikrofon: tarayıcının Web Speech API'si ile konuşmayı Türkçe yazıya çevirir,
  // tanınan metni "Yapılan İşlemler" kutusuna ekler.
  function dinlemeBaslatDurdur() {
    if (!sesDestekli) return;
    if (dinliyor) {
      recognitionRef.current?.stop();
      return;
    }
    ...
    rec.onresult = (e) => {
      let metin = '';
      for (let i = e.resultIndex; i < e.results.length; i++) metin += e.results[i][0].transcript;
      metin = metin.trim();
      if (metin) setGecmisForm((f) => ({ ...f, aciklama: f.aciklama ? `${f.aciklama} ${metin}` : metin }));
    };
  ...
  // Ham (sesle yazılmış) notu AI'a gönderip profesyonel rapora çevirir
  async function raporDuzenle() {
    if (!gecmisForm.aciklama.trim()) return;
    recognitionRef.current?.stop();
    setRaporDuzenleniyor(true);
    setGecmisHata('');
    try {
      const res = await api.post('/bakim/rapor-duzenle', { metin: gecmisForm.aciklama });
      if (res.data.veri?.rapor) setGecmisForm((f) => ({ ...f, aciklama: res.data.veri.rapor }));
    } catch (err) {
      setGecmisHata(err.response?.data?.hata || 'Rapor düzenlenemedi. AI servisi çalışıyor mu?');
    } finally {
      setRaporDuzenleniyor(false);
    }
  }

  // Modalı kapatırken dinlemeyi de durdur
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between p-5 text-white" style={{ background: 'linear-gradient(135deg, #10b981, #047857)' }}>
              ...
              <button onClick={gecmisKapat} className="p-1.5 text-white/70 hover:text-white hover:bg-white/10 rounded-lg transition-colors">
            ...
            <form onSubmit={gecmisKaydet} className="p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-semibold text-slate-600">Yapılan İşlemler *</label>
                  <div className="flex items-center gap-1.5">
                    {/* Mikrofon: sesle yazdır (tarayıcı STT) */}
                    {sesDestekli && (
                      <button
                        type="button"
                        onClick={dinlemeBaslatDurdur}
                      ...
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M19 11a7 7 0 01-14 0m7 7v3m0-3a3 3 0 01-3-3V5a3 3 0 016 0v6a3 3 0 01-3 3z" />
                        </svg>
                        {dinliyor ? 'Dinleniyor...' : 'Sesle Yazdır'}
                      </button>
                    )}
                    {/* AI ile profesyonel rapora çevir */}
                    <button
                      type="button"
                      onClick={raporDuzenle}
                      disabled={raporDuzenleniyor || !gecmisForm.aciklama.trim()}
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold border border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 disabled:opacity-50 transition-colors"
                    ...
                    >
                      ...
                      {raporDuzenleniyor ? 'Düzenleniyor...' : 'AI ile Düzenle'}
                    </button>
                  </div>
                </div>
                <textarea required rows={4} value={gecmisForm.aciklama} onChange={(e) => setGecmisForm({ ...gecmisForm, aciklama: e.target.value })}
                ...
                {dinliyor && (
                  <p className="text-xs text-rose-500 mt-1 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 rounded-full bg-rose-500 animate-pulse" /> Konuşun, sözleriniz yazıya dökülüyor...
                  </p>
                )}
              </div>
              {gecmisHata && (
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={gecmisKapat}
